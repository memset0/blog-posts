...
>
> $1\le n\le 5\times 10^4$，$1\le m\le 10^5$，$1\le w,h,x_i,y_i,l_i,r_i \le 2\times 10^3$。

（本文将介绍一个理论复杂度更优的做法，但由于常数限制，这一做法未能在 QOJ 上通过全部测试集。）
...
之后枚举每个块，上面有若干条离线上来的询问。考虑计算矩形 $[x_i,x_i+w-1] \times [y_i,y_i+h-1]$ 与 $[x_j,x_j+w-1]\times [y_j,y_j+h-1]$ 是否有交，等价于查询 $(x_i,y_i)$ 是否在矩形 $[x_j-w+2,x_j+w-2]\times [y_j-h+2,y_j+h-2]$ 内。将块中的所有**询问**插入到 K-D Tree 中，用块中的 $S$ 个**广告**作为查询，可以利用 K-D Tree 优化建图。利用插入的节点是 $(x_1,y_1),(x_2,y_2) \ldots (x_n,y_n)$ 的子集这一性质，通过预先 $O(n\log n)$ 建树，这里每个块的建树复杂度都是 $O(n)$ 的，而查询贡献的复杂度为 $O(S\sqrt{n})$。这一部分总时间复杂度为 $O\left(\dfrac{n^2}S + n\sqrt{n}\right)$。
