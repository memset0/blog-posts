...
- $b$ is called the image of $a$ undef $f$.
...
A function $f$ from $A$ to $B$ is called **满射函数(onto function)**, or **满射的(surjective)** if
...
> [!important] Useful Properties of the Floor and Ceiling Functions
...
- If there is an _injection_ from $A$ to $B$, the cardinality of $A$ is less than or the same as the cardinality of $B$ and we write $|A| \le |B|$. When $|A| \le |B|$ and $A$ and $B$ have different cardinality, we say that the cardinality of $A$ is less than the cardinality of $B$ and write $|A|<|B|$. 只要能构造一个单射就行，也就是说对于每一个 $A$ 集合中的元素都需要射到一个不同的 $B$ 集合中的元素，但不要求 $B$ 中的每个元素都被射到。
