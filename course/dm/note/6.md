...
1. 自反闭包（Reflexive Closure）：给定一个关系 R，其自反闭包是在 R 上添加元素，使得对于每一个元素 a，都有(a, a)在 R 中。
2. 对称闭包（Symmetric Closure）：给定一个关系 R，其对称闭包是在 R 上添加元素，使得对于每一对元素(a, b)，如果(a, b)在 R 中，那么(b, a)也在 R 中。
3. 传递闭包（Transitive Closure）：给定一个关系 R，其传递闭包是在 R 上添加元素，使得对于每一对元素(a, b)和(b, c)，如果(a, b)和(b, c)在 R 中，那么(a, c)也在 R 中。

> [!summary] Definition: Closure of relation
>
...
> [!important] Theorem
>
> Let $R$ be a relation on $A$. The **自反闭包(reflexive closure)** of $R$, denoted by $r(R)$, is $R \cup I_A$.
>
...
> [!summary] Definition: Connectivity relation
>
> The **联通关系(connectivity relation)** denoted by $R^\ast$, is the set of ordered pairs $(a,b)$ such that there is a path (in $R$) from $a$ to $b$: $R^\ast = \displaystyle{\bigcup_{n=1}^\infty R^n}$.
>
> > [!important] Theorem
> >
