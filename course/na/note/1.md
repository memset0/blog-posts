...
### 1.2. 截断和舍入 | Chopping and Rounding

使用舍入法或截断法产生的误差==都叫做== **舍入误差(roundoff error)**。

- **截断(chopping)**：0.1119 -> 0.111
- **舍入(rounding)**：0.1119 -> 0.112
...
计算 $f(x) = x^3 - 6.1x^2 + 3.2x + 1.5$ 在 $x = 4.71$ 处，使用 3 位有效数字算术。
...
> [!note] ![|350](https://img.memset0.cn/2024/11/15/MjOWhf3Y.png)
>
> $x = (-1)^S \times (1+Fraction) \times 2^{Exponent-Bias}$
>
> - $S$ 代表 **符号位(sign bit)**。$S$ 为 $0$ 表示非负数，$S$ 为 $1$ 表示负数。
> - 有效位数($Significand = 1 + Fraction$) 中 $1$ 是默认的，我们只存储小数点后面的位数(即 Fraction)。
...
> > 5. 分数:
> >
> > - Single: $0.1 = 100\cdots 00$，共 23 位
> > - Double: $0.1 = 100\cdots 00000$，共 52 位
> >   $\therefore$ IEEE 754 表示：
> > - Single: $1\ 01111110\ 100\cdots 00$
> > - Double: $1\ 01111111110\ 100\cdots 00000$

> [!note] IEEE754 浮点数-> 十进制
>
> > [!question] 单精度浮点数 11000000101000…00 代表什么数字
> >
> > 1. 符号位：$S = 1$
> > 2. 指数：$E = 10000001 = 129$
> > 3. 实际指数：$E - 127 = 2$
