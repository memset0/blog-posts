...
- AND: $AB = \overline{\overline{A\cdot B}} = \text{NAND}(\text{NAND}(A,B))$.
- OR: $A+B = \overline{\overline{A}\space\overline{B}} = \text{NAND}(\text{NAND}(A),\text{NAND}(B))$.

将基本逻辑门映射到 NOR 门：

- AND: $AB = \overline{\overline{A}+\overline{B}} = \text{NOR}(\text{NOR}(A),\text{NOR}(B))$.
- OR: $A+B = \overline{\overline{A+B}} = \text{NOR}(\text{NOR}(A,B))$.
...
## 6. 算术函数与电路 Arithmetic Functions and Circuits
...
![bjJQvHv2.png|507](https://static.memset0.cn/img/v6/2024/04/11/bjJQvHv2.png)
...
- The term $X\cdot Y$ is **进位产生函数(carry generate function)**：当 $X$ 和 $Y$ 都是 $1$ 时就会发生进位。一般简写成 $G$。

- The term $X \oplus Y$ is **进位传递函数(carry propagate function)**：当 $X$ 和 $Y$ 中恰有一个是 $1$ 的话，如果收到一个来自低位的进位，就一定会传递给高位。一般简写成 $P$。
...
#### 6.1.3. 行波加法器 Ripple-Carry Binary Adders

![6RdC6Rmk.png|519](https://static.memset0.cn/img/v6/2024/04/11/6RdC6Rmk.png)

#### 6.1.4. 超前进位加法器 Carry Lookahead Adder
...
#### 6.2.1. 无符号减法 Unsigned Subtraction
...
#### 6.2.2. 有符号减法 Signed Subtraction
