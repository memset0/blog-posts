...
![gyONWRkQ.png|575](https://img.memset0.cn/2024/04/18/gyONWRkQ.png)
...
![fal07yxB.png|475](https://img.memset0.cn/2024/04/18/fal07yxB.png)

我们以一个二路选择器为例：

![nYKitFyS.png|569](https://img.memset0.cn/2024/04/18/nYKitFyS.png)

- **毛刺(glitch)**：或称瞬态错误。当 $S$ 从 $1$ 变为 $0$ 时，由于非门有 0.2ns 的延迟，由 $A \overline{S}$

如果把选择器的输出 $Y$ 直接连到输入 $A$ 上呢？

![JR6iiTsg.png|585](https://img.memset0.cn/2024/04/18/JR6iiTsg.png)
...
![bwbO3cDX.png|582](https://img.memset0.cn/2024/06/12/bwbO3cDX.png)

在 SR 锁存器中，$S=1,\ R=1$ 的输入是**禁止的(forbidden)** 或者说**未定义的(undefined)**，此时输出端 $Q=0$ 和 $\overline{Q}=0$ 同时成立。一般不使用这种状态。

S'R' 锁存器通过两个与非门实现。

![n2u8p5CN.png|556](https://img.memset0.cn/2024/06/12/n2u8p5CN.png)
...
![BkfC3qgA.png|304](https://img.memset0.cn/2024/04/18/BkfC3qgA.png)
...
![bHO3VS6o.png|512](https://img.memset0.cn/2024/05/10/bHO3VS6o.png)

一种可行的解决方案是：$C=1$ 时（输入的时候）先不改变输出，等到 $C=0$ 时在改变，这就是主从触发器的原理。

### 2.5. 锁存器的标准符号 Standard Symbols for Latches

![iFJ7MCYp.png|497](https://img.memset0.cn/2024/04/18/iFJ7MCYp.png)

## 3. 触发器 Flip-Flop

### 3.1. 主从触发器 Master-Slave Flip-Flop

![2ksxJEq7.png|351](https://img.memset0.cn/2024/04/18/2ksxJEq7.png)
...
>
> ![WoIhiZRM.png|590](https://img.memset0.cn/2024/04/18/WoIhiZRM.png)
>
...
![hsbdV5kX.png|533](https://img.memset0.cn/2024/06/12/hsbdV5kX.png)
...
![uiJvlAgB.png|560](https://img.memset0.cn/2024/04/18/uiJvlAgB.png)

![EfG4e0AK.png|284](https://img.memset0.cn/2024/04/18/EfG4e0AK.png)
...
![整体模型|332](https://img.memset0.cn/2024/05/10/k6yAYzTF.png)
...
![来自课本 Figure 5-15|300](https://img.memset0.cn/2024/05/10/YOevSc7d.png)

![时序电路分析过程|458](https://img.memset0.cn/2024/06/12/Ulem2F8x.png)
...
![Moore 型电路的状态表|516](https://img.memset0.cn/2024/06/12/3t59xjur.png)
...
![dv7sfKyn.png|506](https://img.memset0.cn/2024/06/12/dv7sfKyn.png)
