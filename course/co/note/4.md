...
> 本篇笔记介绍了处理器的基本架构和实现方式。首先讲解了单周期 CPU 的设计，包括指令执行的五个阶段（IF、ID、EX、MEM、WB）、数据通路和控制信号的设计。随后详细讨论了流水线 CPU 的实现，重点阐述了三种冒险（hazard）：结构冒险、数据冒险和控制冒险，以及相应的解决方案，如前递（forwarding）技术和分支预测。通过对比单周期和流水线两种实现方式，深入理解了处理器的工作原理和性能优化方法。<small style="font-style: italic; opacity: 0.5">（由 claude-3.5-sonnet 生成摘要）</small>
...
通过不同的 **控制信号(control signal)** 控制多个 MUX，可以让在同一条数据通路上实现不同功能的指令。
...
![|438](https://img.memset0.cn/2024/11/16/xeXC6k5s.png)
...
### 2.4. Pipelined Datapath & Control
