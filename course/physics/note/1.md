...
重力势能 $U_{p\text{重}} = mgh$，弹性势能 $U_{p\text{弹}} = \dfrac{1}{2} kx^2$。
...
**质点的角动量**：$\boldsymbol{L} = \boldsymbol{r} \times m \boldsymbol{v} = r m v \sin \theta$，其中 $\theta$ 为速度与法线的夹角。

**质点系的角动量**：$\boldsymbol{L} = \sum \boldsymbol{r_i} \times m \boldsymbol{v_i}$。

### 4.2. 力矩

**力矩** $\boldsymbol{M}$ 表征力 $\boldsymbol{F}$ 使物体绕参考点 $O$（相对于该点取矩）*转动*的能力。

**质点所受力矩**：$\boldsymbol{M} = \boldsymbol{r} \times \boldsymbol{F}$。

### 4.3. 角动量定理

$$
\boldsymbol{M}_{\text{外}} = \frac{\text d\boldsymbol{L}}{\text dt}
$$

### 4.4. 角动量守恒定律

若 $\displaystyle{\sum_i \boldsymbol{M}_{i\text{外}} = 0}$，则 $\boldsymbol{L} = \displaystyle\sum_i \boldsymbol{L}_i = \text{常量}$。
