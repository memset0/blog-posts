...
现在我们讨论反向传播的过程 $p_{\theta}(\mathbf{x}_{t-1}|\mathbf{x}_{t}) = \mathcal{N}(\mathbf{x}_{t-1}; \boldsymbol{\mu}_{\theta}(\mathbf{x}_{t},t), \boldsymbol{\Sigma}_{\theta}(\mathbf{x}_{t},t))$ 中的具体参数化方式。对于 $\boldsymbol{\Sigma}_{\theta}$，本论文中作者认为设计一个较为简单的固定值就能得到较好的效果，后续工作把这一项也优化成可训练的了：
